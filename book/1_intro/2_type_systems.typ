...
那么，为什么要构建类型系统呢？答案显而易见：因为类型系统本身就很精彩，在理论上引人入胜；但其次，因为它们确实很有用。让我们从背景知识开始。如果你想的话，你可以跳过这部分直接去看公式和代码。但在深入理论和实现之前，了解一些软性的背景知识是很有用的。
...
3. 函数应用：现在我们已经有了运用规则 $[T"-App"]$ 所必须的前提。我们将规则中的 $e_1$ 替换为 $f$，$e_2$ 替换为 $x$，$tau_1$ 替换为 $"Int"$，$tau_2$ 替换为 $"Bool"$。因为前提已经在前两步推导中得以证明，现在我们可以形成最终结论：
...
历史上，许多程序语言都强制要求不同抽象层之间严格分离，这种概念被称为#term[分层 (stratification)]。在这一模型中存在一个“#term[词项 (term)] 语言”和一个“类型语言”，它们在语法和概念上泾渭分明。词项语言由能够确实地计算出值、在运行时被执行的表达式构成，例如 $5 + 2$ 或 $"if" x "then" y "else" z$。而类型语言则由描述词项的表达式构成，例如 $"Int"$ 或者 $"Bool" -> "Bool"$。两个世界不可混淆，类型不能出现在词项的位置，反之亦然。
...
而近几十年来，人们对完全类型推断的关注度有所下降。主要原因是，随着类型系统变得越来越强大、复杂，完全类型推断在计算上变得难以处理，在很多时候甚至是不可判定的。现代程序语言通常包含#term[高秩多态 (higher-rank polymorphism, 将多态函数作为参数传递)]、#term[广义代数数据类型 (Generalized Algebraic Data Types, GADTs)]，以及各种类型级编程——在这些类型级编程特性中，类型本身就可能涉及计算。对于这些系统而言，不存在一种能够为任意表达式推断出“唯一最佳类型”的通用算法。试图这样做会导致算法变得极其复杂，推导出的类型也会变得冗长晦涩，令人费解。
...
这确实非常强大，但其代价却很容易被低估。编写证明需要大多数程序员不具备的专业知识。证明的负担可能比实现工作量高出一个数量级。证明辅助工具在自动化方面取得了显著进展，但全自动推导深层正确性的愿景仍遥不可及。支持这些特性的语言仍然比较小众，主要用于关键系统的形式化验证和数学研究。
