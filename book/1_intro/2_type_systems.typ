...
通过定义一系列这样的规则，我们就能创造一个完整的类型系统。每条规则都定义了如何确定某一类表达式——如函数调用、字面值或者 `if-then-else` 块——的类型。例如，函数应用的规则会要求我们先证明如下前提：函数具有函数类型 $T -> U$，且参数具有相应的输入类型 $T$。如果我们能证明这些前提，则规则允许我们得出整个函数应用表达式具有输出类型 $U$。通过反复应用这些规则，我们就能从关于字面量的公理出发，逐步构造出一棵推导树，并最终汇聚成关于整个程序类型的一个大判断，从而证明程序是#term[类型良定的 (well-typed)]。
...
随着机器学习系统越来越多地生成代码，类型系统为建立信任提供了一种途径。类型检查器是一种验证器：它只接受符合其规则的程序，无论代码是人写的，还是大语言模型写的。更丰富的类型系统提高了可接受代码的标准，因而能在部署前捕获更多错误。这为在更富表达力的类型系统上投入资源提供了一个合理的工程论据。但值得注意的是，我们在实际系统中关注的大多数属性，例如性能、抵御侧信道攻击的安全性或分布式状态的正确处理，仍然超出了当前类型系统能够表达的范围。
