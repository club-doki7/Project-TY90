...
然而，现代的强大类型系统设计的一个主要趋势是摈弃这种严格的分层，将词项语言和类型语言统一到一个单一的、一致的语法框架中。在这些统一系统中，“值”（词项）和“描述”（类型）的界限开始变得模糊。语言的文法允许表达式在不同的“层级”或“#term[宇宙 (universe)]”中被解释。例如，宇宙 $"Type"_0$ 中包含 $"Bool"$ 这样的简单类型，而更高层的宇宙 $"Type"_1$ 中唯一的成员就是 $"Type"_0$。这样一来，你就可以编写作用域类型上的函数，这是#term[依值类型 (dependent type)] 语言的一个关键特性，稍后会详细说明。
...
而近几十年来，人们对完全类型推断的关注度有所下降。主要原因是，随着类型系统变得越来越强大、复杂，完全类型推断在计算上变得难以处理，在很多时候甚至是不可判定的。现代编程语言通常包含高阶多态（将多态函数作为参数传递）、广义代数数据类型，以及各种类型级编程——在这些类型级编程特性中，类型本身就可能涉及计算。对于这些系统而言，不存在一种能够为任意表达式推断出“唯一最佳类型”的通用算法。试图这样做会导致算法变得极其复杂，推导出的类型也会变得连篇累牍，令百思不解。
...
- 合成模式：在这个模式下，算法在没有任何期望的情况下，计算——或者说“合成”——表达式 $e$ 的类型。信息从报答是的组件中“向上”流动。我们是在提问，“$e$ 的类型是什么？”
...
- 一个逻辑*命题*对应用编程中的一个*类型*，而类型又对应于范畴论中的*对象*；
...
依值类型则更进一步。在 Agda、Rocq 和 Lean 这样的语言中，你可以定义一个 `SortedList T` 类型，其值为列表以及列表内容已排序的证明。而一个类型签名为 #linebreak() `SortedList T -> SortedList T -> SortedList T` 的函数不仅要合并两个列表，还要给出结果已经排序的证明。编译器会拒绝任何不能提供证明的实现。
...
这里有一个诱人的设想：如果类型是命题，程序是证明，那么或许我们可以明确定义想要的类型，然后让计算机自动合成程序。这就是基于规范进行程序合成的理想状态。然而，现实情况要远没有那么理想。目前的合成工具适用于规模较小、约束较多的问题。如何从复杂规范中生成正确的实现仍是一个开放课题，现有的实用工具也只能处理有限的领域。

随着机器学习系统越来越多地生成代码，类型系统为建立信任提供了一种途径。类型检查器是一种验证器：它只接受符合其规则的程序，无论代码是人写的，还是大语言模型写的。更丰富的类型系统提高了可接受代码的标准，因而能在部署前捕获更多错误。这为投身于开发更强的类型系统提供了一个合理的工程论据。但值得注意的是，我们在实际系统中关注的大多数属性，例如性能、抵御侧信道攻击的安全性或分布式状态的正确处理，仍然超出了当前类型系统能够表达的范围。

逻辑、计算和范畴论之间的理论联系精妙绝伦，几十年来一直指导着编程语言的设计。这些联系表明，这些结构中蕴含着某种深刻而统一的本质。然而，将理论转化为程序员实际可用的工具仍是一项持续的工作。理论上的可能性与实际应用之间存在着巨大的差距，弥合这一差距需要对证明自动化、语言设计和工具开发进行持续的研究。

这些理念值得深入研究，它们或许不会立即改变你的软件编写方式，但它们揭示了代码背后的数学本质。理解类型为何如此运作，以及它们能够表达什么，能让你成为更优秀的程序和语言设计者。这种理解本身就是一种回报。
